...
def cart_id(db_session):
    cart = Cart(user_id=1)
    return create_cart(cart.user_id, db_session)


@pytest.fixture
def cart_with_items(db_session, cart_id):
...
def test_get_cart_by_user_id(db_session, cart_id):
...
def test_remove_all_items(db_session, cart_with_items):
    remove_all_items(cart_with_items, db_session)
    items = db_session.query(CartItems).filter_by(cart_id=cart_with_items).all()
    assert len(items) == 0
    assert items == []


def test_decrement_item_quantity(db_session, cart_id):
...
def test_increment_item_quantity_existing_item(db_session, cart_id):
...
def test_increment_item_quantity_creates_item(db_session, cart_id):
    increment_item_quantity(cart_id, 1, db_session)
    new_item = db_session.query(CartItems).filter_by(cart_id=cart_id, item_id=1).first()
    assert new_item.quantity == 1


def test_load_cart_db(db_session, cart_with_items):
    cart_items = load_cart_db(cart_with_items, db_session)
